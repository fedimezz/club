...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const plan = await prisma.weeklyPlan.findUnique({ where: { id: weeklyPlanId }, select: { id: true, isArchived: true } });
    ...
    const session = await prisma.session.create({
      data: { weeklyPlanId, day, startTime, endTime, activity, coach, coachId: coachId || null, capacity, location: location || "Salle principale", description: description || null },
    });
